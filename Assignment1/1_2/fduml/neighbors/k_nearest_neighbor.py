...
class KNeighborsClassifier(object):
    ...
    def compute_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                dists[i, j] = np.square(np.sum(np.square(X[i]-self.X_train[j])))
    ...
    def predict_labels(self, dists):
        ...
        for i in range(num_test):
            ...
            #########################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            sort_indices = np.argsort(dists, axis=1)[:, 0:self.k]
            ...
            ##########################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            y_pred = np.apply_along_axis(lambda row: np.bincount(row).argmax(), axis=1, arr=closest_y)
