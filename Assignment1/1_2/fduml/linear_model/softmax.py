...
class SoftmaxClassifier(LinearModel):
    ...
    def __init__(self, learning_rate=1e-4, reg=1e-5, num_iters=50, batch_size=200,
    ...
    def fit(self, X, y):
        ...
        for it in range(self.num_iters):
            ...
            indices = np.random.choice(np.arange(num_train), size=self.batch_size, replace=True, p=None)
            X_batch = X[indices, :]
            y_batch = y[indices]
    ...
    def softmax_loss_naive(self, W, X, y, reg):
        ...
        for i in range(self.batch_size):
            ...
            dW += np.dot(X[i][:, np.newaxis], softmax[np.newaxis, :])
    ...
    def softmax_loss_vectorized(self, W, X, y, reg):
        ...
        row_sums = np.sum(scores_exp, axis=1, keepdims=True)
        softmax = scores_exp / row_sums

        X_sorted = scores[np.arange(self.batch_size), y]
        X_sorted = X_sorted[:, np.newaxis]
        # 这一行或许有更好的解决方法？

        loss = np.sum(softmax - X_sorted) + self.reg*(np.sum(W**2))/2
        dW = np.dot(X.T, softmax) + self.reg*W
