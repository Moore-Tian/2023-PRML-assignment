...
class BCELoss(Loss):
    ...
    def forward(self, input, target):
        ...
        loss = -np.sum(self.target * np.log(input) + (1 - self.target) * np.log(1 - input)) / input.shape[0]
    ...
    def backward(self):
        ...
        ###########################################################################
        input_grad = 1 / (1 + np.exp(-input)) - self.target
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
class CrossEntropyLoss(Loss):
    ...
    def __init__(self, model, label_num, reduction = 'mean') -> None:
        self.input = None
        self.target = None
    ...
    def forward(self, input, target):
        self.input = input
        target = np.eye(self.label_num)[target.numpy()]
        self.target = target
        ...
        softmax = input_exp / row_sums
        losses = np.sum(-np.log(softmax) * target, axis=input.ndim - 1)
        loss = np.mean(losses)
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        return loss, softmax
    ...
    def backward(self):
        ...
        row_sums = np.sum(input_exp, axis=input.ndim - 1, keepdims=True)
        softmax = input_exp / row_sums
        input_grad = softmax - self.target
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        self.model.backward(input_grad)
